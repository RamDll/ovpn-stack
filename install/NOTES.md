...
**только файл ключа** в `~/.ssh/ovpn-stack-<ip>` (`chmod 600`) + `.pub`.
Сводка и финальная строка печатают
`ssh -i ~/.ssh/ovpn-stack-<ip> -p <port> <user>@<ip>`. Флаг
`--no-key-copy` выключает — тогда ключ только в `.state/`, и об этом
явно пишется в сводке.

`~/.ssh/config` скрипт **не трогает** — личный файл пользователя, алиас
`Host` заводится вручную при желании. Первая версия этого фикса писала
блок по маркерам в `~/.ssh/config`; по требованию владельца оставили
только копию ключа. `state.env` (пароли sudo/панелей) в `~/.ssh` не
копируется — остаётся в `.state/` и в тексте сводки.
...
2026-09-06 `setup.sh` на «Шаге 6» сам копирует файл ключа в
`~/.ssh/ovpn-stack-<ip>` (`~/.ssh/config` не трогает) — подключение по
готовой команде из сводки, клон после установки не нужен. Отключается
`--no-key-copy`.
