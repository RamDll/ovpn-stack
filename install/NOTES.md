...
`setup.sh` на «Шаге 6» теперь по умолчанию (`install_ssh_config`):
- копирует ключ в `~/.ssh/ovpn-stack-<ip>` + `.pub`;
- добавляет в `~/.ssh/config` блок между `# BEGIN ovpn-stack <ip>` /
  `# END ovpn-stack <ip>` (`Host ovpn-stack-<ip>` → HostName/User/Port/
  IdentityFile/IdentitiesOnly). Прежний блок вырезается awk-ом по
  маркерам, хвостовые пустые строки схлопываются — повторные прогоны
  не растят файл, чужие записи не трогаются;
- сводка и финальная строка печатают `ssh ovpn-stack-<ip>`.

Флаг `--no-ssh-config` выключает (для managed `~/.ssh/config`) — тогда
ключ только в `.state/`, и об этом явно пишется в сводке. `state.env`
(пароли) в `~/.ssh` не копируется — остаётся в `.state/` и в сводке.

Раньше в README стояло «алиас в `~/.ssh/config` не создаём — это личный
файл пользователя»; решение пересмотрено, потому что блок по маркерам
не конфликтует с ручными записями, а альтернатива — молча потерять
сервер при `rm -rf` клона.
...
2026-09-06 `setup.sh` на «Шаге 6» сам копирует ключ в
`~/.ssh/ovpn-stack-<ip>` и пишет блок `Host ovpn-stack-<ip>` в
`~/.ssh/config` (маркеры `# BEGIN/END ovpn-stack <ip>`, повторный
прогон перезаписывает) — подключение `ssh ovpn-stack-REDACTED-IP`,
клон после установки не нужен. Отключается `--no-ssh-config`.
