...
Флаги:
  ...
  --sni <domain>   свой домен для REALITY SNI (иначе — случайный из
                   встроенного пула; при повторном прогоне берётся из
                   state.env). Роль домена чисто визуальная, см. NOTES.md §9
...
# HTTPS-сайты на TLS 1.3, которые почти нигде не блокируют. SNI берётся
# случайным из пула без вопроса: dest давно локальный (NOTES §9), сервер
# к домену не ходит, роль чисто визуальная — осознанный выбор смысла не
# имеет, а одинаковый у всех SNI — лишний признак для DPI. Свой домен,
# если есть причина, — флаг --sni.
...
if [[ "$MODE" == "vless" || "$MODE" == "all" ]]; then
  ...
  if [[ -z "$DEST" ]]; then
    # SNI — имя, которое клиент шлёт в своём ClientHello; для DPI трафик
    # выглядит как HTTPS к этому сайту. Сервер к домену НЕ подключается
    # (dest — локальная заглушка, NOTES §9), поэтому выбираем случайный
    # из пула без вопроса. Фиксируется в state.env, при повторном прогоне
    # переиспользуется (смена = перевыпуск всех розданных ссылок).
    # Свой домен, если нужен, — флаг --sni.
    DEST="${SNI_POOL[RANDOM % ${#SNI_POOL[@]}]}"
    info "SNI: $DEST — случайный из пула (роль визуальная; свой — флаг --sni)"
    save_state   # зафиксировать выбор сразу: повторный прогон не перевыберет
  else
...

{
  ...
  if [[ -n "$SUB_PATH" ]]; then
    ...
    echo "Остальным клиентам ссылку/QR бери прямо в панели (карточка клиента)."
    echo
  fi
  echo "== Напоминания =="
