...
export function useUserActions() {
  ...
  return {
    create: (username: string, expireDays = 0) =>
      run(() => ovpn.createUser(username, expireDays), 'toast.userCreated', username),

    rotate: (username: string, expireDays = 0) =>
      run(() => ovpn.rotateUser(username, expireDays), 'toast.certRotated', username),
