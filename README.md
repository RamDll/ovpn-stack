...
3. **спрашивает**: домен *или* публичный IP (для TLS), логин и пароль панели
   (пустой — сгенерирует), UDP-порт OpenVPN (по умолчанию `7777`), каталог
   установки (по умолчанию `/opt/ovpn-stack`), e-mail для Let's Encrypt;
